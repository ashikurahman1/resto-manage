...
const UserLayout = ({ children }) => {
  ...
  const menuItems = [
    ...
    { name: 'My Orders', href: '/dashboard', icon: <MdShoppingBag size={24} />, exact: true },
    { name: 'Profile', href: '/dashboard/profile', icon: <MdSettings size={24} /> },
  ];
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-base-200">
      ...
      <aside className="hidden lg:flex w-72 bg-base-100 border-r border-base-300 flex-col sticky top-0 h-screen shadow-sm">
        <div className="p-8 flex items-center gap-3 border-b border-base-200">
          ...
          <span className="text-xl font-black text-base-content tracking-tighter uppercase">Member</span>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 p-2 md:p-10 overflow-y-auto pb-24 lg:pb-10">
        {children}
      </main>
    </div>
  );
